...
class BangBangTrajectory1D(Trajectory):
    ...
    def generate(self, initial_pos: float, final_pos: float, initial_vel: float, max_vel: float, max_acc: float,
        ...
        if target_time is None or target_time < 0:
            ...
            self.check_and_append_parts(
                BangBangTrajectory1D.generate_shortest_timed(initial_pos, final_pos, initial_vel, max_vel, max_acc,
                                                             target_time),
                no_fail=True
            )
        return self
    ...
    def generate_shortest_timed(initial_pos: float, final_pos: float, initial_vel: float, max_vel: float,
        ...
        def finish_up(parts: List[BBTrajectoryPart]):
            t_diff: float
            if len(parts) > 1:
                t_diff = parts[-1].t_end - parts[-2].t_end
            else:
                t_diff = parts[-1].t_end
            v0 = parts[-1].v0 + parts[-1].acc * t_diff
            s0 = parts[-1].s0 + 0.5 * (parts[-1].v0 + v0) * t_diff

            a = - math.copysign(max_acc, v0)

            if math.isclose(parts[-1].acc, 0.0) or math.isclose(parts[-1].acc, -a):
                parts.append(BBTrajectoryPart())
                parts[-1].acc = a
                parts[-1].v0 = v0
                parts[-1].s0 = s0
                parts[-1].t_end = parts[-2].t_end - v0 / a
            elif math.isclose(parts[-1].acc, a):
                parts[-1].t_end += -v0 / a
            else:
                raise ValueError("Unexpected Value for a = {}".format(parts[-1].acc))

            return parts

        can_reach, fastest_parts, _, time_diff = BangBangTrajectory1D.can_reach(initial_pos, final_pos, initial_vel,
                                                                                max_vel, max_acc, target_time)

        if not can_reach:
            return finish_up(fastest_parts)
        else:
            ...
            if shortest[-1].t_end - ACCURACY < target_time:
                return shortest
            BangBangTrajectory1D.slow_down_fastest_simple(fastest_parts, initial_pos, final_pos, initial_vel, max_acc,
                                                          target_time)
            return finish_up(fastest_parts)
    ...
    def can_reach(initial_pos: float, final_pos: float, initial_vel: float, max_vel: float, max_acc: float,
        ...
        if math.isclose(final_pos - initial_pos, 0.0, abs_tol=1e-4) and math.isclose(initial_vel, 0.0, abs_tol=1e-4):
            ...
            part.v0 = 0
            part.s0 = initial_pos
            return True, [part], "stand-still", target_time
    ...
    def slow_down_fastest_simple(parts: List[BBTrajectoryPart], initial_pos: float, final_pos: float,
        ...
        if len(parts) == 2:
            assert math.isclose(parts[-1].acc, 0.0, abs_tol=1e-3), "Unexpected a = {}".format(parts[-1].acc)
            # https://www.wolframalpha.com/input?i=solve+v_0*t_1+%3Dv_0*t_2%2B1%2F2*a*Power%5Bt_2%2C2%5D%2Bv_1*t_3%2C+v_1+%3D+v_0%2Ba*t_2%2C+t_1%2Bt%3Dt_2%2Bt_3+for+v_1%2Ct_1%2C++t_2
        ...
        if len(parts) < 2:
            parts.append(BBTrajectoryPart())
        parts[1].acc = -a
        parts[1].v0 = parts[0].v0 + parts[0].acc * t1
        parts[1].s0 = parts[0].s0 + 0.5 * (parts[0].v0 + parts[1].v0) * t1
        parts[1].t_end = t
        return
